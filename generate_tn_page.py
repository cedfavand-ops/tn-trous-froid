...
INFOCLIMAT_STATIONS = [
    {"name": "St Christol d'Albion", "meta": "Vaucluse (84) · 824 m", "code": "STATIC0257"},
    {"name": "Lachapelle-Graillouse – Moulin de Courbet", "meta": "Ardèche (07) · 1134 m", "code": "STATIC0407"},
    {"name": "Combe de l'Oscence", "meta": "La Chapelle-en-Vercors (26) · 975 m", "code": "000UF"},
    {"name": "Combe de Darbounouse", "meta": "La Chapelle-en-Vercors (26) · 1282 m", "code": "STATIC0213"},
    {"name": "Doline de Solaison", "meta": "Brizon (74) · 1479 m", "code": "STATIC0305"},
    {"name": "Reculfoz", "meta": "Doubs (25) · 1010 m", "code": "STATIC0015"},
]
...
def fetch_infoclimat_official(codes, token):
    """
    Interroge l'API officielle Infoclimat OpenData (un seul appel pour toutes
    les stations) et retourne un dict {code: (tn_ou_None, statut)}.
    """
    if not token:
        return {c: (None, "token Infoclimat manquant") for c in codes}
    ...
    for c in codes:
        params.append(("stations[]", c))

    try:
        r = requests.get("https://www.infoclimat.fr/opendata/", params=params, timeout=30)
        try:
            data = r.json()
        except ValueError:
            return {c: (None, f"réponse non-JSON (statut {r.status_code}): {r.text[:200]!r}") for c in codes}

        if isinstance(data, dict) and data.get("request_state") not in (None, 200) and "hourly" not in data:
            msg = data.get("message", str(data)[:200])
            return {c: (None, f"API erreur (statut {r.status_code}): {msg}") for c in codes}

        # La structure exacte peut varier selon la version de l'API ; on cherche,
        # pour chaque code de station, toute valeur numérique associée à un champ
        # dont le nom contient "temp" (température), en excluant humidité/vent/etc.
        results = {}

        def find_station_block(obj, code):
            """Cherche récursivement le sous-objet associé à un code de station."""
            if isinstance(obj, dict):
                if code in obj:
                    return obj[code]
                for v in obj.values():
                    found = find_station_block(v, code)
        ...
        def collect_temps(obj):
            ...
            def walk(o, key_hint=""):
                if isinstance(o, dict):
                    ...
                    if "temp" in key_hint.lower() and "humid" not in key_hint.lower():
                        try:
        ...
        for code in codes:
            block = find_station_block(data, code)
            if block is None:
                results[code] = (
                    None,
                    f"station absente de la réponse API (clés racine: {list(data.keys()) if isinstance(data, dict) else type(data)}, extrait: {str(data)[:300]!r})",
                )
                continue
            temps = collect_temps(block)
            if not temps:
                results[code] = (None, f"aucune valeur de température trouvée (extrait: {str(block)[:150]!r})")
                continue
            results[code] = (min(temps), "ok")

        return results

    except Exception as e:
        return {c: (None, f"erreur: {e}") for c in codes}
...
def main():
    ...
    infoclimat_token = os.environ.get("INFOCLIMAT_TOKEN", "")

    rows_html = []
    ok_count = 0
    total = len(INFOCLIMAT_STATIONS) + len(METEORAVANEL_STATIONS) + len(DATACAKE_STATIONS) + len(ECOWITT_STATIONS)

    infoclimat_codes = [st["code"] for st in INFOCLIMAT_STATIONS]
    infoclimat_results = fetch_infoclimat_official(infoclimat_codes, infoclimat_token)

    for st in INFOCLIMAT_STATIONS:
        tn, status = infoclimat_results.get(st["code"], (None, "non interrogé"))
        if tn is not None:
            ok_count += 1
        else:
