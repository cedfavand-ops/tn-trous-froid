...
def fetch_infoclimat_tn(url: str):
    ...
    try:
        ...
        r.raise_for_status()
        html = r.text
        # Le bloc "Température minimale" est suivi d'une valeur en gras: **X.X**°C
        m = re.search(r"Temp[ée]rature minimale.{0,400}?\*\*(-?\d+(?:\.\d+)?)\*\*\s*°C", html, re.S)
        if not m:
            # variante sans markdown (HTML brut avec balises <b>)
            m = re.search(r"Temp[ée]rature minimale.{0,400}?<b>(-?\d+(?:\.\d+)?)</b>\s*°C", html, re.S)
        if m:
            return float(m.group(1)), "ok"
        return None, "valeur introuvable sur la page"
...
def fetch_meteoravanel_tn(url: str):
    ...
    try:
        ...
        r.raise_for_status()
        html = r.text
        m = re.search(r"Temp[ée]rature minimale [aA]ujourd.hui:\s*(-?\d+(?:\.\d+)?)\s*°C", html)
        if m:
            return float(m.group(1)), "ok"
        return None, "valeur introuvable sur la page"
    except Exception as e:
...
def fetch_datacake_tn(device_id: str, token: str):
    ...
    try:
        ...
        data = r.json()
        # La forme exacte de la réponse peut varier ; on cherche toute liste de valeurs numériques
        values = []
        def collect(obj):
        ...
        collect(data)
        if not values:
            return None, "aucune donnée renvoyée"
        return min(values), "ok"
    except Exception as e:
        return None, f"erreur: {e}"
